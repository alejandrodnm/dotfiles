...
vim.api.nvim_set_keymap('n', '<Leader>gm', ':Gdiff main <bar> let g:gitgutter_diff_base = "main"<CR>', { silent = true })
...
vim.api.nvim_set_keymap('n', '<Leader>h', '<cmd>lua require("dash.providers.telescope").dash({ bang = false, initial_text = "" })<cr>', {})
...
-- Rg command
vim.cmd("command! -nargs=* Rg call fzf#vim#grep('rg --column --line-number --no-heading --color=always --smart-case ' .. shellescape(<q-args>), 1, { 'options': '--delimiter : --nth 2..' })")
